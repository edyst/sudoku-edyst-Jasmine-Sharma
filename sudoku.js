    ...
    if(sessionStorage.getItem("cache")){
        ...
        if(diff==1) {
            ...
            if(saved[i]!="-" && saved[i]!=difficulty[i]){
                document.querySelectorAll(".cell")[i].classList.add("modified"); 
...
function highSelect() {
    for(let i=0;i<81;i++) {
        document.querySelectorAll(".cell")[i].classList.remove("highlighted");
        document.querySelectorAll(".cell")[i].classList.remove("dup");
    }
    ...
    for(let i=0;i<81;i++){
        ...
        document.getElementById(i).classList.add("dup");
    }
    ...
    if( (-1<s && s<3) || (8<s && s<12) || (17<s && s<21) ) {    
        for(let i=x;i<y;i++){
            for(let j=i;j<i+19;j+=9){
                if(j!=s) {
                    document.getElementById(j).classList.add("highlighted");
                }
                }
            }
        }
...
function addToArray(num) {
    ...
    document.querySelector(".selected").classList.add("modified");
    duplicate(num);
}
function removeFromArray() {
    let index = document.querySelector(".selected").id;
    answer.splice(index,1,"-");
    document.querySelector(".selected").classList.remove("modified");
}
